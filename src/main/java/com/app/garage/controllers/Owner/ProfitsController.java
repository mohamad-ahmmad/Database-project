...
public class ProfitsController implements Initializable  {
    ...
    private void searchClicked(ActionEvent e) throws SQLException{
        String depId = searchField.getText();
        if(depId.length() !=3){
            searchField.setStyle("-fx-border-color: rgba(248,0,0,0.6)");
            return;
        }
        int temp;
        try{
            temp =  Integer.parseInt(depId);
        }catch(Exception ex){searchField.setStyle("-fx-border-color: rgba(248,0,0,0.6)"); return;}
        
        con = DriverManager.getConnection(App.ip, App.user, App.password);
        Statement st = con.createStatement();
        ResultSet found = st.executeQuery("select DID from DEPARTMENT where DID="+temp);
        if(found.next()){
            searchAll(temp);
            calculateDepIdLineChart(temp);
            pieChartCalculation(temp);
